...
export default class GeocodingRepository {
  static async geocodeYandex(params: IYandexGeocodeRequest): Promise<YandexResponseGeocoder> {
    const query = {...params,
      ...
      ll: params.ll?.length && params.ll?.length > 1 ? `${params.ll[0]},${params.ll[1]}` : undefined,
      spn: params.spn?.length && params.spn?.length > 1 ? `${params.spn[0]},${params.spn[1]}` : undefined,
...
9
  ...
  static async suggestYandex(params: IYandexSuggestRequest): Promise<IYandexSuggestItem[]> {
    ...
    return new Promise((resolve, reject) => {
      jsonp(`https://suggest-maps.yandex.ru/suggest-geo?${ queryString.stringify(query)}`, undefined, (err: any, data: any) => {
        if (err) {
          reject(err.message)
        } else {
