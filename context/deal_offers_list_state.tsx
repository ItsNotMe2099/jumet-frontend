...
const DealOfferListContext = createContext<IState>(defaultValue)
...
export function DealOfferListWrapper(props: Props) {
  ...
  const fetch = async ({page}: { page: number } = {page: 1}) => {
    ...
    try {
      const res = await DealOfferRepository.fetch({
        ...filterRef.current,
  ...
  return (
    <DealOfferListContext.Provider value={value}>
      {props.children}
    </DealOfferListContext.Provider>
  )
}

export function useDealOfferListContext() {
  return useContext(DealOfferListContext)
}
